...
let userID = 'aaa495fd-a618-47f7-919b-2820850cacdc';
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        const proxySettings = await env.Avan.get('proxySettings', {type: 'json'});
...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `💦 Avan - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `💦 Avan - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `💦 Avan - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `💦 Avan - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '💦 Avan Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestFragment.remarks = '💦 Avan Frag - Best Fragment 😎';
...
const getWarpConfigs = async (env, client) => {
    ...
    xrayWarpBestPing.remarks = '💦 Avan - Warp Best Ping 🚀'
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `💦 Avan - WoW ${index/2 + 1} 🌍`;
...
const fetchWgConfig = async (env, warpKeys) => {
    ...
    await env.Avan.put('warpConfigs', JSON.stringify(warpConfigs));
}
...
const updateDataset = async (env, Settings) => {
    ...
    try {
        currentProxySettings = await env.Avan.get("proxySettings", {type: 'json'});
    } catch (error) {
        console.log(error);
...
const renderHomePage = async (env, hostName, fragConfigs) => {
    ...
    try {
        proxySettings = await env.Avan.get('proxySettings', {type: 'json'});
        warpConfigs = await env.Avan.get('warpConfigs', {type: 'json'});
        password = await env.Avan.get('pwd');
    } catch (error) {
        console.log(error);
	...
	<body>
		...
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#Avan-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}#Avan-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}#Avan-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/mehdint/Avan-Worker-Panel-main" target="_blank">Github</a>
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/mehdint/Avan-Worker-Panel-main">documents</a>' 
                    : ''}
                </h2>
